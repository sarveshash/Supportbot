...
BOT_TOKEN = '7160432819:AAFvslOSUwvksivIXcGNtK27dki2CJkieYc'
...
class Database:
    ...
    def connect(self):
        try:
            self.conn = connect(self.db_file, check_same_thread=False)
            return self.conn
        except Error as e:
            print(e)
...
c.execute('''CREATE TABLE IF NOT EXISTS submitted_users (
                username TEXT PRIMARY KEY
            )''')
conn.commit()
...
# Check if the clumn already exists in the user_data table
...
# Check if the team_name column already exists in the user_data table
    



admin_ids = ['1381668733', '1985764612', '5048444272' , '7025637676' , '7125783665']
updated_list = []
updated2_list = []
added_number = []
submitted_users = []
...
team_conn = sqlite3.connect('team_data.db')
team_c = team_conn.cursor()

# Create a table to store user IDs and team names
team_c.execute('''CREATE TABLE IF NOT EXISTS user_teams (
                    user_id TEXT PRIMARY KEY,
                    team_name TEXT
                )''')
team_conn.commit()

@client.on(events.NewMessage(pattern='/setteam'))
async def set_team_command(event):
    if event.is_reply:
        reply_msg = await event.get_reply_message()
        user_id = str(reply_msg.sender_id)
        team_name = event.raw_text.split(maxsplit=1)[1]  # Extract the team name from the command

        # Store the user's team name in the team data database
        team_c.execute("INSERT OR REPLACE INTO user_teams (user_id, team_name) VALUES (?, ?)", (user_id, team_name))
        team_conn.commit()

        await event.reply(f"Team name set to: {team_name}")
...
async def profile_command(event):
    ...
    team_c.execute("SELECT team_name FROM user_teams WHERE user_id = ?", (sender_id,))
    team_data = team_c.fetchone()

    if user_data:
        user_id, username, added_number, profile_data = user_data[:4]  # Extract relevant user data
        team_name = team_data[0] if team_data else "Not set"  # Extract team name or set default message

        red_team_name = f"\U0001F534 {team_name}"  # Unicode character for red square followed by the team name

        user_info = f"User info:\n"
        user_info += f"ID: {user_id}\n"
        user_info += f"First Name: {username}\n"
        user_info += f"Added Number: {added_number}\n"
        user_info += f"Profile Data: {profile_data}\n"
        user_info += f"Team Name: {red_team_name}"  # Display the team name in red

        await event.reply(user_info)
    else:
        await event.reply("You need to set up your profile first.")

@client.on(events.NewMessage(pattern='/0'))
async def delete_setteam_data(event):
    sender_id = str(event.sender_id)

    # Delete the user's team data from the database
    team_c.execute("DELETE FROM user_teams WHERE user_id = ?", (sender_id,))
    team_conn.commit()

    await event.reply("Your team data has been deleted.")
